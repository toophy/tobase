...
func (m *StateMachine) ExportWithDetails(outfile string, format string, layout string, scale string, more string) error {
	...
	file, _ := os.Create("fsm.dot")
	file.WriteString(dot)
	file.Close()
	cmd := fmt.Sprintf("dot fsm.dot -o%s -T%s -K%s -s%s %s", outfile, format, layout, scale, more)
...
func system(c string, dot string) error {
	if runtime.GOOS == "windows" {
		...
		cmd := exec.Command("cmd.exe", "/c", "start", "/min")
		cmd.Args = append(cmd.Args, cs...)
